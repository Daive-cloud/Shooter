...
namespace QFramework
{
    ...
    public interface IArchitecture
    {
        ...
        IUnRegister RegisterEvent<T>(Action<T> onEvent); // 都是有参数的
        void UnRegisterEvent<T>(Action<T> onEvent);

        void Deinit();
    }
    /// <summary>
    /// (CTPR)自递归泛型，要求T必须是Architecture<T>的子类，且包含一个公共无参构造函数，这样可以强类型的调用派生类中的成员
    /// 如果有这个泛型约束，编译器就可以知道T一定是派生自Architecture基类的，那么就可以在基类中使用T调用基类的方法，因为有继承关系是一定可以访问到的
    /// </summary>
    /// <typeparam name="T"></typeparam>

    public abstract class Architecture<T> : IArchitecture where T : Architecture<T>, new() // 泛型约束表明T必须是Architecture<T>的子类，且包含一个公共无参构造函数
    {
        private bool mInited = false;   

        public static Action<T> OnRegisterPatch = architecture => { };  // 此时编译器会自动把OnRegisterPatch的类型推断为T

        protected static T mArchitecture; // 可以安全的实例化并管理该架构的唯一实例
    ...
    public abstract class AbstractQuery<T> : IQuery<T>
    {
        ...
        protected abstract T OnDo(); // 这是唯一需要强制重写的方法
    ...
    public struct CustomUnRegister : IUnRegister
    {
        ...
        public CustomUnRegister(Action onUnRegister) => mOnUnRegister = onUnRegister; // 将事件从外界传入后赋值
    ...
    {
        private Action mOnEvent = () => { }; // 空的一个事件委托列表

        public IUnRegister Register(Action onEvent)
        {
            mOnEvent += onEvent; // 将委托加入多播委托列表
